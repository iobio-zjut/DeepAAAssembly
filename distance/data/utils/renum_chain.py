...
def get_sequence_from_pdb(pdb_file):
    ...
    for record in SeqIO.parse(fh, 'pdb-atom'):
        chain_id = record.id  # Assuming record.id contains the chain identifier
        chain_id = chain_id.split(':')[1]
        sequence_dict[chain_id] = str(record.seq)
...
def process_pdbfile(in_file, out_file):
    ...
    fhandle = delete_elements(fhandle, 'TER')#删除所有包含 'TER' 的行，这通常是用于标记 PDB 文件的链的末尾。
    fhandle = alter_chain(fhandle, '9')#调用一个函数 alter_chain，该函数将文件句柄中所有的链标识（Chain ID）替换为 'A'
    fhandle = renumber_residues(fhandle, 1)#调用一个函数 renumber_residues，该函数将文件句柄中的氨基酸残基重新编号，从 1 开始。
...
if __name__ == '__main__':
    ...
    parser.add_argument('-al', '--alter_chain', help="Alter chain or not", type=str,default='True')
    ...
    if option == 'process_pdbfile':
        ...
        fhandle = delete_elements(fhandle, 'HETATM')
        fhandle = delete_elements(fhandle, 'ANISOU')
        if ac_flag =='true':
            fhandle = alter_chain(fhandle, '9')
        fhandle = renumber_residues(fhandle, 1)
        fhandle = renumber_atom_serials(fhandle, 1)
        write_pdb_file(fhandle, out_file)
    elif option == 'get_sequence_from_pdb_and_save':
        sequence_dict = get_sequence_from_pdb(pdb_file)
        if len(sequence_dict.keys()) >= 2:

            name_base, chain1, chain2 = name.split('_')

            sequence1 = sequence_dict.get(chain1, "Chain1 sequence not found")
            sequence2 = sequence_dict.get(chain2, "Chain2 sequence not found")

            name1 = name_base+'_'+chain1
            name2 = name_base+'_'+chain2
            os.makedirs(os.path.join(out_folder,name1),exist_ok=True)
            os.makedirs(os.path.join(out_folder,name2),exist_ok=True)
            out_file1 = f'{out_folder}/{name1}/{name1}.fasta'
            out_file2 = f'{out_folder}/{name2}/{name2}.fasta'

            open(out_file1, 'w').write(f'>|{name1}|\n{sequence1}\n')
            open(out_file2, 'w').write(f'>|{name2}|\n{sequence2}\n')
