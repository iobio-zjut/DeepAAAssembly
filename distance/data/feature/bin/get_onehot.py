...
def seq_embedding(target,a3m_dir_path,txt_folder):
    with open(a3m_dir_path, 'r') as file:
        lines = file.readlines()
    sequence = lines[1].strip()
    length = len(sequence)
    result_array = np.zeros((20, length), dtype=float)
    # 字典映射
    ...
    for seq_index, aa in enumerate(sequence):
        ...
        if aa not in AA_to_num:
            continue
        # 获取对应的数字
        channel = AA_to_num[aa]
        # 将数组的对应位置置为1.0
        result_array[channel, seq_index] = 1.0
    embedding_file = f'{txt_folder}/{target}_seqembedding.txt'
    with open(embedding_file, "w") as file:
        pass
    np.savetxt(embedding_file, result_array, fmt='%.1f')
...
if __name__ == "__main__":
    ...
    seq_embedding(target,a3m_dir_path,txt_folder)
