...
for name in "${list_name[@]}"; do
  if [ ! -d "$target_dir/${name}/tbm/trans_mat_5model_bestscore" ]; then
    echo "Running domain assembly (tbm)...${name}"
    mkdir $target_dir/${name}/tbm/trans_mat_5model_bestscore
    cp $target_dir/flags $target_dir/${name}/tbm
    cp $target_dir/stage1.sh $target_dir/${name}/tbm
    cd $target_dir/${name}/tbm
    chmod 755 $target_dir/${name}/tbm/stage1.sh
    ./stage1.sh &
